...
# 🎟️ SmartBookingBackend

This is the backend for the SmartBooking application, built with Node.js, Express, and MongoDB. It serves as the API layer for managing users, events, bookings, wishlists, billing, and more.
...
- 🎫 Event search & detail retrieval (concerts, sports, comedy, etc.)
- 📅 User bookings (upcoming & past)
...
This project uses **Jest** for unit testing. All major API routes and controllers have associated test files located in `src/tests`.
...
> The coverage report will be shown in the terminal and saved in the `coverage/` directory.

### 🔍 Run a Specific Test File

To test a specific module, for example `billing`:
...
```bash
npx jest src/tests/billing.test.ts --coverage
```
...
└── tests/
    ...
    ├── myTicket.test.ts
    ├── OtherController.test.ts
    ├── others.test.ts
    ├── purchase.test.ts
    ├── search.test.ts
    ├── searchService.test.ts
    ├── ticketmasterApi.test.ts
    └── wishlist.test.ts
```

> ✅ Each file contains unit tests for the corresponding route/controller logic and ensures isolated coverage of API behavior.
